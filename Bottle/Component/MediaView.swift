...
struct MediaView: View {
    ...
    @Environment(\.albumID) var albumID

    var thumbnailURL: String? {
        if entities.image?.localThumbnailURL != nil {
            entities.image?.localThumbnailURL
        } else {
            entities.media.thumbnailUrl
        }
    }
                ...
                if let work = entities.work {
                    withInfoView {
                        image.resizable().scaledToFit()
                            .draggable(DraggableWork(image: image, work: work))
                    }
                } else {
                    withInfoView {
                        image.resizable().scaledToFit()
    ...
    var contextMenu: some View {
        ...
        if let albumID = albumID, let album = appModel.album(albumID) {
            Button("Remove from Album \"\(album.name)\"") { removeFromAlbum(albumID) }
        }
        ...
        if users.count == 1 {
            ...
            Button(
                "Browse \"\(user.name ?? user.userId)\" Posts at \(user.community.capitalized)", systemImage: "globe"
            ) {
                browsingUser = user
    ...
    private func removeFromAlbum(_ albumID: Album.ID) {
        if let workID = entities.work?.id {
            Task {
                do {
                    try await Client.removeWorks(albumId: albumID, workIds: [workID])
                    model.deleteMedia(entities.media.id)
                } catch {
                    print(error)
                }
            }
    ...
    var inspectorPanel: some View {
        List {
            Section("Media") {
                ...
                if let link = mediaLink(media: media, post: entities.post, user: entities.user) {
            ...
            if let image = entities.image {
                Section("Image") {
                    ...
                    if let width = image.width, let height = image.height {
                        LabeledContent("Dimension", value: "\(width)×\(height)")
                    }
        ...
        Task {
            ...
            do {
                if imported {
                    ...
                    let response = try await Client.addWork(
                        community: media.community, postID: media.postId, page: media.pageIndex)
